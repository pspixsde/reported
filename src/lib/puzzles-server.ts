...
import type { BuildClashPuzzle, Puzzle } from "./game-types";
import { getRedis } from "./redis";

const PUZZLES_FILE = resolve(process.cwd(), "src/data/puzzles.json");
const CLASH_PUZZLES_FILE = resolve(process.cwd(), "src/data/clash-puzzles.json");
const HEROES_FILE = resolve(process.cwd(), "src/data/heroes.json");

const KV_PUZZLES_KEY = "puzzles:all";
const KV_CLASH_PUZZLES_KEY = "puzzles:clash-all";

// ── In-memory cache (avoids re-fetching on every request within the same lambda) ──
let cachedPuzzles: Puzzle[] | null = null;
let cachedClashPuzzles: BuildClashPuzzle[] | null = null;
let cachedHeroNames: Record<number, string> | null = null;
...
export async function getAllClashPuzzles(): Promise<BuildClashPuzzle[]> {
  if (cachedClashPuzzles) return cachedClashPuzzles;

  const redis = getRedis();
  if (redis) {
    const data = await redis.get<BuildClashPuzzle[]>(KV_CLASH_PUZZLES_KEY);
    if (data && data.length > 0) {
      cachedClashPuzzles = data;
      return data;
    }
  }

  if (!existsSync(CLASH_PUZZLES_FILE)) {
    throw new Error(
      `Clash puzzles not found in KV or at ${CLASH_PUZZLES_FILE}. Run seed:puzzles first.`,
    );
  }

  const puzzles: BuildClashPuzzle[] = JSON.parse(
    readFileSync(CLASH_PUZZLES_FILE, "utf-8"),
  );
  cachedClashPuzzles = puzzles;
  return puzzles;
}

export async function getClashPuzzleById(
  id: string,
): Promise<BuildClashPuzzle | undefined> {
...
export function getHeroNameById(heroId: number): string | undefined {
  if (!cachedHeroNames) {
    if (!existsSync(HEROES_FILE)) return undefined;
    try {
      const heroes = JSON.parse(readFileSync(HEROES_FILE, "utf-8")) as Record<
        string,
        { name?: string }
      >;
      cachedHeroNames = {};
      for (const [id, hero] of Object.entries(heroes)) {
        ...
        if (!Number.isNaN(parsed) && hero?.name) {
          cachedHeroNames[parsed] = hero.name;
        }
      }
    } catch {
      return undefined;
